...
TEST_TITLE="Ensure permissions on /etc/group- are configured"
TEST_ID=060108
FILE="/etc/group-"
...
check_file() {
    ...
    if [ "$numeric_current_permissions" -le "$numeric_expected_permissions" ]; then
        if [ "$current_user" != "$expected_user" ]; then
            echo "$TEST_ID:$TEST_TITLE:1:Incorrect owner on $file. Found $current_user, expected $expected_user"
            return 1
        elif [ "$current_group" != "$expected_group" ]; then
            echo "$TEST_ID:$TEST_TITLE:1:Incorrect group on $file. Found $current_group, expected $expected_group"
            return 1
        fi
        echo "$TEST_ID:$TEST_TITLE:0"
        return 0
    else
        echo "$TEST_ID:$TEST_TITLE:1:Permissions on $file are less restrictive than expected. Found $current_permissions, expected at most $expected_permissions"
        return 1
    fi
}
