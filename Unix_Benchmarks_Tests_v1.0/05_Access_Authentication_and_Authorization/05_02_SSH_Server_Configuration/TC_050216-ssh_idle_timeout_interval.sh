...
check_ssh_idle_timeout() {
    ...
    if [[ -z "$interval" || "$interval" -lt "$MIN_CLIENT_ALIVE_INTERVAL" || "$interval" -gt "$MAX_CLIENT_ALIVE_INTERVAL" ]]; then
        echo "$TEST_ID:$TEST_TITLE:1:ClientAliveInterval is not correctly configured. Found $interval, Expected between $MIN_CLIENT_ALIVE_INTERVAL and $MAX_CLIENT_ALIVE_INTERVAL."
        return 1
    fi

    if [[ -z "$count_max" || "$count_max" -gt "$MAX_CLIENT_ALIVE_COUNT_MAX" ]]; then
        echo "$TEST_ID:$TEST_TITLE:1:ClientAliveCountMax is not correctly configured. Found $count_max, Expected $MAX_CLIENT_ALIVE_COUNT_MAX or less."
        return 1
    fi
