...
TEST_TITLE="Ensure unsuccessful unauthorized file access attempts are collected"
TEST_ID=040110
...
# Define expected rules for monitoring unsuccessful unauthorized file access attempts
expected_rules=(
    "-a always,exit -F arch=b$([ "$ARCH" = "x86_64" ] && echo 64 || echo 32) -S creat -S open -S openat -S truncate -S ftruncate -F exit=-EACCES -F auid>=1000 -F auid!=4294967295 -k access"
    "-a always,exit -F arch=b$([ "$ARCH" = "x86_64" ] && echo 64 || echo 32) -S creat -S open -S openat -S truncate -S ftruncate -F exit=-EPERM -F auid>=1000 -F auid!=4294967295 -k access"
)
