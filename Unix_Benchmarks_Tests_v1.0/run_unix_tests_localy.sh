...
MAIN_DIR="${ROOT_DIR}/Unix_Benchmarks_Tests_v1.0" # Ścieżka do katalogu z testami

# Pobierz aktualny czas i sformatuj go do postaci RRRRMMDDHHMMSS
TIMESTAMP=$(date +"%Y%m%d%H%M%S")

# Pobierz tylko nazwę dystrybucji systemu
os_name=$(cat /etc/*release | grep -w NAME | cut -d "=" -f2 | tr -d '"' | tr ' ' '_')
...
# Funkcja do przetwarzania wyników testów i zapisywania ich do pliku CSV
process_test_result() {
    local result=$1
    IFS=':' read -r -a array <<< "$result"
    local test_id="${array[0]}"
    ...
    #echo -e "[ ${color}${status_text}${RESET} ] ${WHITE}TC-$test_id${RESET}: ${PURPLE}$test_title${RESET}. ${CYAN}$additional_info${RESET}"
    echo -e "${WHITE}TC-$test_id${RESET} [ ${color}${status_text}${RESET} ] : ${PURPLE}$test_title${RESET}. ${CYAN}$additional_info${RESET}"
...
execute_scripts_in_directory() {
    ...
    if [ "$(ls -A ${directory})" ]; then
        for script in ${directory}/*.sh; do
            result=$($script) # Zmienione wywołanie skryptu
