...
tests_dir="$dir_path/cis_deb_11_sec_audit_tests"
...
if [ -n "$output_file_name" ]; then
    ...
    chown administrator:administrator "$output_file"
...
for test_script in "${test_scripts[@]}"; do
    original_permissions=$(stat -c "%a" "$test_script") # Zapisz oryginalne uprawnienia
    chmod +x "$test_script" # Zmień uprawnienia na wykonywalne

    ((total_tests++))
    test_output=$("$test_script" 2>&1)
    test_failed=$(echo "$test_output" | grep -q "FAIL;" && echo "yes" || echo "no")

    chmod "$original_permissions" "$test_script" # Przywróć oryginalne uprawnienia
