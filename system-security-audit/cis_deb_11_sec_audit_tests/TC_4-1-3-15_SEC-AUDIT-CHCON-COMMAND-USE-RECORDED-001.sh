...
if ! command -v auditctl &> /dev/null; then
    ...
    if [ -z "${UID_MIN}" ]; then
        ...
        expected_rule="-S all -F path=/usr/bin/chcon -F perm=x -F auid>=${UID_MIN} -F auid!=-1 -F key=perm_chng"

        # Sprawdzanie obecności oczekiwanej reguły
        if ! echo "$loaded_rules" | grep -Pq -- "$(echo $expected_rule | sed 's/[\&\/]/\\&/g')" 2>/dev/null; then
            test_fail_messages+=("Brak oczekiwanej reguły audytu dla polecenia chcon.")
            exit_status=1
        fi
    fi
fi
