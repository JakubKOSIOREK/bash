...
if [ -f /etc/audit/auditd.conf ]; then
    ...
    if [ -n "$incorrect_perms_files" ]; then
        test_fail_message=$(echo "$incorrect_perms_files" | tr '\n' ';')
        echo "FAIL;$test_id;$test_file;$test_name;Niektóre pliki dziennika audytu nie mają ustawionych uprawnień 0640 lub niższych.;$test_fail_message"
        exit_status=1
    else
