...
expected_rule="-a always,exit -F path=/usr/bin/setfacl -F perm=x -F auid>=${UID_MIN} -F auid!=-1 -k perm_chng"

# Sprawdzanie reguł na dysku
if ! grep -Pq -- "$(echo $expected_rule | sed 's/[\&\/]/\\&/g')" /etc/audit/rules.d/*.rules 2>/dev/null; then
    echo "FAIL;$test_id;$test_name; - Brak oczekiwanej reguły audytu dla polecenia setfacl."
    exit_status=1
fi

# Sprawdzanie załadowanych reguł
if ! auditctl -l | grep -Pq -- "$(echo $expected_rule | sed 's/[\&\/]/\\&/g')" 2>/dev/null; then
    echo "FAIL;$test_id;$test_name; - Oczekiwana reguła dla polecenia setfacl nie jest załadowana."
    exit_status=1
fi
