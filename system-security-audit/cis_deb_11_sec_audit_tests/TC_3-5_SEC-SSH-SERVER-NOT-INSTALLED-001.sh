...
test_id="SEC-SSH-SERVER-NOT-INSTALLED-001"
test_name="Ensure SSH Server is not installed"
script_path="$0"
...
packages_to_check="openssh-server openssh-client dropbear lsh-server sshfs putty putty-tools"
...
if dpkg-query -W -f='${binary:Package}\t${Status}\t${db:Status-Status}\n' $packages_to_check 2>/dev/null | grep -Pi '\h+installed\b' >/dev/null; then
    installed_packages=$(dpkg-query -W -f='${binary:Package}\n' $packages_to_check 2>/dev/null | grep -v 'no packages found matching' | tr '\n' ';')
