...
if echo "$apparmor_status_output" | grep -E "^[[:digit:]]+ profiles are in complain mode." | grep -qvE "0 profiles are in complain mode."; then
...
if echo "$apparmor_status_output" | grep -E "^[[:digit:]]+ processes are unconfined" | grep -qvE "0 processes are unconfined"; then
