...
TEST_TITLE="Ensure ICMP redirects are not accepted (Host and Router)"
TEST_ID=030302
EXPECTED_VALUE=0
...
if [ "$ipv4_all_result" -eq 0 ] && [ "$ipv4_default_result" -eq 0 ]; then
    echo "$TEST_ID:$TEST_TITLE:0"
    exit 0
else
    # If any check returned 'not implemented', return 'not implemented'
    if [ "$ipv4_all_result" -eq 2 ] || [ "$ipv4_default_result" -eq 2 ]; then
        exit 2
    else
        exit 1
    fi
