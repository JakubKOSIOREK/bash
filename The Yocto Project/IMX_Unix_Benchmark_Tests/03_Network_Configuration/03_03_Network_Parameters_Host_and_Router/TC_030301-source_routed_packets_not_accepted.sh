...
TEST_TITLE="Ensure source routed packets are not accepted (Host and Router)"
TEST_ID=030301
EXPECTED_VALUE=0
...
if [ "$all_ipv4_result" -eq 0 ] && [ "$default_ipv4_result" -eq 0 ]; then
    echo "$TEST_ID:$TEST_TITLE:0"
    exit 0
else
    # If any check returned 'not implemented', return 'not implemented'
    if [ "$all_ipv4_result" -eq 2 ] || [ "$default_ipv4_result" -eq 2 ]; then
        exit 2
    else
        exit 1
    fi
